...
#     dplyr::summarise(p_na = mean(is.na(value)), .groups = "drop") %>% 
...
#   p_df %>% 
#     group_by(year_start) %>% dplyr::summarise(n_new = length(unique(idpers)),.groups="drop") %>% 
#     ggplot(aes(x=year_start,y=n_new))+
#     geom_bar(stat="identity")
#   
#   
#   p_df %>% 
#     filter(year_start %in% c(2014:2014),sex=="woman",age_start %in% 30:40) %>% 
#     dplyr::select(idhous,idpers,age,pp21,year) %>% 
#     pivot_wider(id_cols = idpers,names_from = year,values_from=pp21) %>% 
#     arrange(is.na(`2014`)) %>% View()
#   
#   p_df %>% filter(idpers==60022102) %>% View()
#   
#   #mean
#   df = p_df %>% 
#     filter(year_start %in% c(2006:2015),sex=="woman",age_start %in% 20:40) %>% 
#     dplyr::mutate(dplyr::across(everything(), as.character)) %>% 
#     dplyr::mutate(row_id = dplyr::row_number()) %>% 
#     tidyr::pivot_longer(cols = -c(year,row_id), names_to = "var", values_to = "value") %>% 
#     left_join(variables_df,by="var") %>% 

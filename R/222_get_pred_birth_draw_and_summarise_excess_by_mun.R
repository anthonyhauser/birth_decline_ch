#not used
get_pred_birth_draw_and_summarise_excess_by_mun = function(fit, #cmdstanr fit
                                       stan_df,
  ...
  #sum over age (by year, mun_id and draw)
  ...
  excess_birth_year_adj_mun_draw_df = adj_table %>% 
    ...
    group_by(year,mun_id,mun_name,draw) %>% 
    dplyr::summarise(adj_n_pred = sum(adj_n_pred),
                     adj_n_exc = sum(adj_n_exc),.groups="drop_last")
  excess_birth_year_adj_mun_df = excess_birth_year_adj_mun_draw_df %>% 
    #summarise by year and mun_id
    dplyr::summarise(n_exp_mean = mean(adj_n_pred),
                     n_exc_mean = mean(adj_n_exc),
                     n_exc_lwb = quantile(adj_n_exc,probs=0.025),
                     n_exc_upb = quantile(adj_n_exc,probs=0.975),.groups="drop") %>% 
  ...
  #adjacent, 2017:2024
  excess_birth_adj_mun_draw_df = adj_table %>% 
    #add mun_name
    left_join(excess_birth_year_mun_draw_df %>% dplyr::select(mun_id,mun_name) %>% distinct(),
              by="mun_id") %>% 
    #add n_pred and n_exc
    ...
    dplyr::summarise(adj_n_pred = sum(adj_n_pred),
                     adj_n_exc = sum(adj_n_exc),.groups="drop_last")
  excess_birth_adj_mun_df = excess_birth_adj_mun_draw_df%>% 
    #summarise by year and mun_id
    dplyr::summarise(n_exp_mean = mean(adj_n_pred),
                     n_exc_mean = mean(adj_n_exc),
                     n_exc_lwb = quantile(adj_n_exc,probs=0.025),
    ...
    dplyr::mutate(rel_exc_mean = n_exc_mean/n_exp_mean,
                  rel_exc_lwb = n_exc_lwb/n_exp_mean,
                  rel_exc_upb = n_exc_upb/n_exp_mean) 
  saveRDS(excess_birth_adj_mun_draw_df, paste0("results/",save.date,"_",mod_name,"_","seedid",seed_id,"_","excess_birth_adj_mun_draw_df",".RDS"))
  ...
  excess_birth_year_mun_df = excess_birth_year_mun_draw_df %>% 
    ...
    dplyr::summarise(n_birth = n_birth[1],
                     ...
                     n_exp_upb = quantile(n_pred,probs=0.975),
                     n_exc_mean = mean(n_exc),
                     n_exc_lwb = quantile(n_exc,probs=0.025),
                     n_exc_upb = quantile(n_exc,probs=0.975),.groups="drop") %>% 
    dplyr::mutate(rel_exc_mean = (n_birth - n_exp_mean)/n_exp_mean,
                  rel_exc_lwb = (n_birth- n_exp_lwb)/n_exp_mean,
                  rel_exc_upb = (n_birth-n_exp_upb)/n_exp_mean)  #%>% 
    # left_join(pop_mun_df %>%
    #             group_by(year,mun_id) %>% 
    #             dplyr::summarise(n_pop=sum(n_pop),.groups="drop"),by=c("year","mun_id")) 
  ...
  if(FALSE){
    ...
    excess_birth_adj_mun_draw_df = readRDS(paste0("results/",save.date,"_",mod_name,"_","seedid",seed_id,"_","excess_birth_adj_mun_draw_df",".RDS"))
    
    excess_birth_year_mun_df = readRDS(paste0("results/",save.date,"_",mod_name,"_","seedid",seed_id,"_","excess_birth_year_mun_df",".RDS"))
    excess_birth_year_adj_mun_df = readRDS(paste0("results/",save.date,"_",mod_name,"_","seedid",seed_id,"_","excess_birth_year_adj_mun_df",".RDS"))
    excess_birth_adj_mun_df = readRDS(paste0("results/",save.date,"_",mod_name,"_","seedid",seed_id,"_","excess_birth_adj_mun_df",".RDS"))
  }
  ...
  return(list(excess_birth_year_mun_draw_df = excess_birth_year_mun_draw_df,
              ...
              excess_birth_adj_mun_draw_df = excess_birth_adj_mun_draw_df,
              excess_birth_year_adj_mun_df = excess_birth_year_adj_mun_df,
              excess_birth_adj_mun_df = excess_birth_adj_mun_df,
              excess_birth_year_mun_df = excess_birth_year_mun_df))
}
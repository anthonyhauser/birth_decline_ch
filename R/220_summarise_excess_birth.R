...
summarise_excess_birth_ctz = function(pred_n_birth_ctz_draw_df,
  ...
  excess_birth_year_ctz_df  = pred_n_birth_ctz_draw_df %>% 
    #sum over age and month
    group_by(citizenship,year,draw) %>% 
    dplyr::summarise(n_pop = sum(n_pop),
                     n_birth = sum(n_birth),
                     n_pred = sum(n_pred),.groups="drop_last") %>% 
    dplyr::summarise(n_pop = n_pop[1],
    ...
    dplyr::mutate(n_exc_mean = n_birth - n_exp_mean,
                  ...
                  rel_exc_upb = (n_birth-n_exp_upb)/n_exp_mean)
  #age
  excess_birth_year_age_ctz_df  = pred_n_birth_ctz_draw_df %>% 
    #sum over age and month
    group_by(citizenship,year,age,draw) %>% 
    dplyr::summarise(n_pop = sum(n_pop),
                     n_birth = sum(n_birth),
                     n_pred = sum(n_pred),.groups="drop_last") %>% 
    dplyr::summarise(n_pop = n_pop[1],
  ...
  excess_birth_year_month_ctz_df  = pred_n_birth_ctz_draw_df %>% 
    #sum over age and month
    group_by(citizenship,year,month,draw) %>% 
    dplyr::summarise(n_pop = sum(n_pop),
                     n_birth = sum(n_birth),
                     n_pred = sum(n_pred),.groups="drop_last") %>% 
    dplyr::summarise(n_pop = n_pop[1],
  ...
  setDT(pred_n_birth_ctz_draw_df)
  
  excess_birth_year_month_age_ctz_df <- 
    pred_n_birth_ctz_draw_df[,# summarise (mean and quantiles)
                             .(n_pop       = n_pop[1],
                               n_birth     = n_birth[1],
                               n_exp_mean  = mean(n_pred),
                               n_exp_lwb   = quantile(n_pred, 0.025),
  ...
  return(list(excess_birth_year_ctz_df = excess_birth_year_ctz_df,
              excess_birth_year_age_ctz_df = excess_birth_year_age_ctz_df,
              excess_birth_year_month_ctz_df = excess_birth_year_month_ctz_df,
              excess_birth_year_month_age_ctz_df = excess_birth_year_month_age_ctz_df))
}

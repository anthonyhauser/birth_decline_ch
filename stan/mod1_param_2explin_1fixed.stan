//parametric model with exp(linear change) for peak age and peak height (it is already log-transformed) and fixed birth_prob_sigma (i.e., not time-varying)
functions {
...
data {
  ...
  array[2] real p_log_h_peak2;     // Prior for yearly GP scale
...
model {
  ...
  log_h_peak2 ~ normal(p_log_h_peak2[1],p_log_h_peak2[2]);
  birth_prob_sigma1 ~ normal(p_birth_prob_sigma1[1],p_birth_prob_sigma1[2]);

  if(inference==1){
    target += neg_binomial_2_log_lpmf(n_birth |log_birth_prob(to_vector(age_id), age_peak1 * exp(age_peak2 * (to_vector(year_id)-1)/(N_age-1)),//even if it's exp(linear) and not linear anymore, we keep the same prior distribution for age_peak2 because if exp(x) similar to x when x is low
                                                              log_h_peak1 + log_h_peak2 * (to_vector(year_id)-1)/(N_age-1),
...
generated quantities{
  ...
  for(i in 1:N_year){
    for(j in 1:N_age){
      birth_prob[i,j] = exp(log_birth_prob2(j,  age_peak1 * exp(age_peak2 * (i-1)/(N_age-1)),
                                            log_h_peak1 + log_h_peak2 * (i-1)/(N_age-1),
                                            birth_prob_sigma1));
    }
  }
}

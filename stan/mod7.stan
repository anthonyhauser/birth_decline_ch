//mod5 with year random effect and heteroscedasticity with GP
functions {
...
transformed data {
  ...
  real s_year = sqrt(N_year1 / (N_year1 - 1.0));
  ...
  matrix[N_age,M_age] PHI_sigma = PHI_EQ(N_age, M_age, L_age, xn1);
  //f_age
...
parameters {
  ...
  sum_to_zero_vector[N_year1] b_year;
...
transformed parameters {
  ...
  diagSPD_sigma = diagSPD_EQ(alpha_sigma, lambda_sigma, L_age, M_age);
  ...
  //sigma
  vector[N_age] sigma = inv(inv_sigma) * exp(f_sigma);
}
model {
  //priors
 ...
 //GP: variance and lengthscale
  ...
  if(inference==1){
    for(i in 1:N){
      target += neg_binomial_2_log_lpmf(n_birth[i] | f_age[age_id[i],year_id1[i]] + 
                                                     b_year[year_id1[i]] + 
                                                     gamma_month[month_id[i]] +
                                                     log_mean_n_birth_n_pop + log(n_pop[i]),
                                                     sigma[age_id[i]]);
    }
  }
}
  ...
  array[N] int n_birth_pois_pred;
  {
    ...
    for(i in 1:N){
     log_mu[i] = f_age[age_id[i],year_id1[i]] + gamma_month[month_id[i]] + log_mean_n_birth_n_pop + 
                 beta_year_rng[year_id1[i]] +  log(n_pop[i]);
     sigma2[i] = sigma[age_id[i]];
    }

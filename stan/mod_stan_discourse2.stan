...
model {
 ...
 //GP: variance and lengthscale
  ...
  if(inference==1){
    target += neg_binomial_2_log_lpmf(n_birth | f_age[age_id] + log_mean_n_birth_n_pop + log(n_pop),
                                             sigma[sigma_id]);
...
generated quantities{
  ...
  array[N] int n_birth_pois_pred;
  {
    for(i in 1:N_year1){
      for(j in 1:N_age){
         birth_prob[i,j] =  exp(f_age[j] + log_mean_n_birth_n_pop);
      }
    }
    vector[N] log_mu;
    for(i in 1:N){
      log_mu[i] = f_age[age_id[i]] +log_mean_n_birth_n_pop+ log(n_pop[i]);
    }

...
model {
 ...
 //GP: variance and lengthscale
  ...
  if(inference==1){
    //target += neg_binomial_2_log_lpmf(n_birth | log(inv_logit(delta0+f_age[age_id])) + log(n_pop), sigma[sigma_id]);
  ...
  array[N] int n_birth_pois_pred;
  {
    for(i in 1:N_year1){
      for(j in 1:N_age){
         ...
         //birth_prob[i,j] =  inv_logit(f_age[j] + delta0);
         birth_prob[i,j] =  exp(f_age[j] + delta0);
      }
    }
    vector[N] log_mu;
    for(i in 1:N){
      //log_mu[i] = log(inv_logit(f_age[age_id[i]] + delta0)) + log(n_pop[i]);

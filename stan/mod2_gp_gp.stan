...
parameters {
  ...
  vector <lower=0>[N_age] inv_sigma;
...
transformed parameters {
  ...
  vector[N_age] sigma = inv(inv_sigma);
...
model {
  ...
  if(inference==1){
    ...
    target += neg_binomial_2_log_lpmf(n_birth | log(inv_logit(logit_birth_prob(N, N_age, age_id, lw, f_year[year_id1], logit_birth_prob_int))) + log(n_pop), sigma[age_id]);
...
generated quantities{
  ...
  array[N] int n_birth_pred = neg_binomial_2_log_rng(log_mu, sigma[age_id]);

...
model {
 ...
 //GP: variance and lengthscale
  ...
  if(inference==1){
    ...
    vector[N] v2; 
    for(i in 1:N){
     v[i] = f_age[age_id[i],year_id1[i]] + b_year[year_id1[i]];
     v2[i] = gamma_month[month_id[i]];
    }
    target += neg_binomial_2_log_lpmf(n_birth | v + v2 + log_mean_n_birth_n_pop + log(n_pop), sigma[sigma_id]);

functions {
  ...
  vector log_birth_prob(vector age_id, vector a_peak, vector h_peak, vector sigma) {
    return log(h_peak) - (age_id - a_peak)^2 .* inv(2*square(sigma));
  }
...
data {
  ...
  array[2] real p_birth_prob_sigma2;    // Prior for yearly GP lengthscale
...
parameters {
  ...
  real birth_prob_sigma2;
}
transformed parameters {
  real sigma = inv(inv_sigma);
  
  vector[N] h_peak;
  vector[N] birth_prob_sigma;
  for(i in 1:N){
    h_peak[i] = max([0.0001,0.0001 + h_peak1+ h_peak2 * (year_id[i])]);
    birth_prob_sigma[i] = max([0.0001,0.0001 + birth_prob_sigma1+ birth_prob_sigma2 * (year_id[i])]);
  }
}
model {
  ...
  birth_prob_sigma2 ~ normal(p_birth_prob_sigma2[1],p_birth_prob_sigma2[2]);
  ...
  for(i in 1:N_year){
    for(j in 1:N_age){
      birth_prob[i,j] = exp(log_birth_prob2(j, age_peak1 + age_peak2 * i,
                                            h_peak1+ h_peak2 * i,
                                            birth_prob_sigma1 + birth_prob_sigma2 * i));
    }
  }
}

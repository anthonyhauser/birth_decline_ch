//parametric model with linear change over time of peak age, exp(linear change) for peak height (as it is already log-transformed) and fixed birth_prob_sigma (i.e., not time-varying)
functions {
...
generated quantities{
  ...
  for(i in 1:N_year){
    for(j in 1:N_age){
      birth_prob[i,j] = exp(log_birth_prob2(j,  age_peak1 + age_peak2 * (i-1)/(N_age-1),

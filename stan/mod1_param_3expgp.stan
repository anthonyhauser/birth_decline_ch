functions {
  ...
  vector log_birth_prob(vector age_id, vector a_peak, vector log_h_peak, vector sigma) {
...
model {
  ...
  if(inference==1){
    target += neg_binomial_2_log_lpmf(n_birth |log_birth_prob(to_vector(age_id), age_peak1 * exp(f_year[1,year_id]/N_age),//scale by N_age might decrease the risk of divergences
                                                              ...
                                                              birth_prob_sigma1 * exp(f_year[3,year_id]/N_age)) + log(n_pop), sigma);
  }
}
  ...
  for(i in 1:N_year){
    for(j in 1:N_age){
      birth_prob[i,j] = exp(log_birth_prob2(j, age_peak1 * exp(f_year[1,i]/N_age),
                                            ...
                                            birth_prob_sigma1 * exp(f_year[3,i]/N_age)));
    }
  }
}
